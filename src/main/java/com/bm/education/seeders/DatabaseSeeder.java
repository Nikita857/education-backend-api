...
public class DatabaseSeeder {
    ...
    public void seedDatabase() {
        ...
        // Ensure skills exist (might be handled by ExtendedDatabaseSeeder, but let's be
        // safe or assume they exist)
        // We will check counts before seeding

        if (userSkillRepository.count() == 0 && skillRepository.count() > 0) {
            createDefaultUserSkills();
        }

        if (adaptationProgramRepository.count() == 0) {
            createDefaultAdaptationPrograms();
        }
